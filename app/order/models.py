...
class Order(models.Model):
    class Order_Status(models.TextChoices):
        PAYMENT_WAIT = 'PW', ('결제 대기')
        PAYMENT_COMPLETE = 'PC', ('결제 완료')
        DELIVERY_READY = 'DR', ('배송 준비')
        DELIVERY_COMPLETE = 'DC', ('배송 완료')

    class Delivery_Type(models.TextChoices):
        COMMON_POST = 'CP', ('샛별 배송')
        POST = 'PO', ('택배 배송')
    ...
    3. 배송지 정보 -
    """
    delivery = models.PositiveIntegerField(default=0, null=True)
    ...
    status = models.CharField(max_length=2, choices=Order_Status.choices, default=Order_Status.PAYMENT_WAIT,
                              help_text='주문 상태')
    ...
    delivery_type = models.CharField(choices=Delivery_Type.choices, max_length=2)
